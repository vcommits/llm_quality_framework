...
class GeminiProvider(LLMProvider):
    def get_model(self) -> BaseChatModel:
        return ChatGoogleGenerativeAI(model=self.model_name, google_api_key=os.getenv("GEMINI_API_KEY"), **self.kwargs)


class GrokProvider(LLMProvider):
    def get_model(self) -> BaseChatModel:
        return ChatOpenAI(model_name=self.model_name, openai_api_key=os.getenv("GROK_API_KEY"),
                          openai_api_base="https://api.x.ai/v1", **self.kwargs)


class TogetherProvider(LLMProvider):
    def get_model(self) -> BaseChatModel:
        return ChatTogether(model=self.model_name, together_api_key=os.getenv("TOGETHER_API_KEY"), **self.kwargs)


class ProviderFactory:
    MODEL_TIERS = {
        'openai': {'lite': 'gpt-4o-mini', 'mid': 'gpt-4o', 'full': 'gpt-4o'},
        'anthropic': {'lite': 'claude-3-haiku-20240307', 'mid': 'claude-3-5-sonnet-20240620',
                      'full': 'claude-3-opus-20240229'},
        'gemini': {'lite': 'gemini-1.5-flash-latest', 'mid': 'gemini-1.5-pro-latest', 'full': 'gemini-1.5-pro-latest'},
        'grok': {'lite': 'grok-2-mini', 'full': 'grok-2'},
        'together': {
